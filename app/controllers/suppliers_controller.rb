...
# display the claims which is under selected supplier
  ...
  # nill =nily.change(month: 4).beginning_of_month

  free_dis_pending_claims = FreeDiscount.where(supplier_id:params[:supplier_id]).pending_claims(nill, nil, true).fetch(:datas)
  ...
  exp_dam_pending_claims = ExpiryDamage.where(supplier_id:params[:supplier_id]).pending_claims(nill, nil, true).fetch(:datas)
  ...
  pur_retr_pending_claims = PurchaseReturn.where(supplier_id:params[:supplier_id]).pending_claims(nill, nil, true).fetch(:datas)
  ...
  rate_chg_pending_claims = RateChange.where(supplier_id:params[:supplier_id]).pending_claims(nill, nil, true).fetch(:datas)
  ...
  non_find_pending_claims = NonFindableClaim.where(supplier_id:params[:supplier_id]).pending_claims(nill, nil, true).fetch(:datas)
