module ClaimData
	...
	class_methods do
		...
		def pending_claims(start_date = nil, end_date = nil)
			...
			datas = query.where(amount_status:nil, claim_status: "0").where.not(ws_settle_amount: "").where('ws_settle_amount::float +10 < claim_amount::float')
			...
			return {count: count_and_sum[0] + zero_paid[0], amount: ((count_and_sum[1].to_f + zero_paid[1].to_f).to_f - count_and_sum[2].to_f).to_f.round(2), datas: datas + zero_paid_datas}
			puts "============================================================================"
		end
