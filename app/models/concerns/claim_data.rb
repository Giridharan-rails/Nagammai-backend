module ClaimData
	...
	class_methods do
		...
		def pending_claims(start_date = nil, end_date = nil, data = nil)
			query = start_date && end_date ? where( Hash[ (has_attribute?(:ack_date) ? "ack_date" : "claim_date").to_sym, start_date..end_date]) : where(nil)
			datas = query.where(amount_status:nil, claim_status: [nil, "0"]).where("ws_settle_amount IN ('') OR ws_settle_amount IS NULL OR ws_settle_amount::float +10 < claim_amount::float")
		...
		def setteld_claims(start_date = nil, end_date = nil)
			query = start_date && end_date ? where( Hash[ (has_attribute?(:ack_date) ? "ack_date" : "claim_date").to_sym, start_date..end_date]) : where(nil)
			datas = query.where(amount_status:nil, claim_status: "0").where.not(ws_settle_amount: "").where('ws_settle_amount::float >= claim_amount::float -10')
